...
axios.defaults.baseURL = "https://alumnicommunity-backend.onrender.com";
  ...
  async (postAndComment, thunkAPI) => {
    try {
      ...
      const response = await axios.put(
        `/posts/${postAndComment.postId}/comment/${postAndComment.commentId}`,
        { message: postAndComment.message }
      );
      return response.data;
    } catch (err) {
      return thunkAPI.rejectWithValue(err.response.data.message);
...
export const findFeed = createAsyncThunk("posts/feed", async (_, thunkAPI) => {
  try {
    setAuthHeader(thunkAPI);
